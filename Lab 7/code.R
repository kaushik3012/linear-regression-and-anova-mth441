...
# Compute the condition number from the covariance matrix of predictors
...
eigen_vectors
# Verify if the eigenvectors match with Table 9.6 of Montgomery
# You will need to manually compare these values with Table 9.6
...
# (d) Calculate Variance Decomposition Proportions (VDP)
calculate_vdp <- function(eigen_vectors, eigen_values, vifs) {
  ...
  for (j in 1:p) {
    for (i in 1:p) {
      ...
      vdp_matrix[j, i] <- (t_ji^2 / lam_i) / vifs[j]  # Calculate the VDP
    }
  }
